...
void testProcess(Rectangle &rect) {
  ...
  rect.setHeight(10);
  cout << endl
       << "expect area : " << wd * 10 << " output area : " << rect.area()
       << endl;
}
...
public:
  static Rectangle CreateReactangle(int width, int height) {
...
int main() {
  // Should avoid funciton such as setHeight in square class , added SquareEx to
  // fix this kind of issue but still square can not substitute rectangle
  // smoothly
  Rectangle rect(5, 5);
  testProcess(rect);
  Square square(5);
