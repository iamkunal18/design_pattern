...
struct ColorSpecification : ISpecification<Product> {
  ...
  bool is_satisfied(Product *p) { return p->color == color; }
};
struct SizeSpecification : ISpecification<Product> {
  Size size;
  explicit SizeSpecification(Size s) { size = s; }
  bool is_satisfied(Product *p) { return p->size == size; }
};
struct AndSpecification : ISpecification<Product> {
  ISpecification<Product> &first;
  ISpecification<Product> &second;
  explicit AndSpecification(ISpecification<Product> &f,
                            ISpecification<Product> &s)
      : first(f), second(s) {}
